...
@Composable fun ExamScanApp(vm:AppViewModel){
    ...
    NavHost(nav,"home"){
        ...
        composable("edit/{id}"){b->PageImageEditor(vm,nav,b.arguments!!.getString("id")!!.toLong())}
    }
}
...
@Composable fun ExamScreen(vm:AppViewModel,nav:NavHostController,examId:Long){
    val exam by vm.exam(examId).collectAsState(initial=null);val papers by vm.papers(examId).collectAsState(initial=emptyList());val activity=LocalActivity.current ?: return;val scope=rememberCoroutineScope();var menu by remember{mutableStateOf(false)};var error by remember{mutableStateOf<String?>(null)};var exporting by remember{mutableStateOf(false)};var readyExport by remember{mutableStateOf<ReadyExport?>(null)}
    val saveLauncher=rememberLauncherForActivityResult(StartActivityForResult()){result->val target=result.data?.data;val source=readyExport;if(result.resultCode==Activity.RESULT_OK&&target!=null&&source!=null){scope.launch{try{withContext(Dispatchers.IO){activity.contentResolver.openInputStream(source.uri)!!.use{input->activity.contentResolver.openOutputStream(target)!!.use{input.copyTo(it)}}};Diagnostics.log(DiagnosticCategory.SHARING,"export_saved",mapOf("type" to source.type));readyExport=null}catch(t:Throwable){Diagnostics.log(DiagnosticCategory.STORAGE,"export_save_failed",error=t);error=t.message?:"Save failed"}}}}
    var pending by remember{mutableStateOf<ScanMode?>(null)};var bulkActive by remember{mutableStateOf(false)};var saving by remember{mutableStateOf(false)}
    val launcher=rememberLauncherForActivityResult(StartIntentSenderForResult()){r->if(r.resultCode==Activity.RESULT_OK){val result=GmsDocumentScanningResult.fromActivityResultIntent(r.data);val uris=result?.pages?.map{it.imageUri}.orEmpty();Diagnostics.log(DiagnosticCategory.ML_KIT,"scanner_result",mapOf("mode" to pending?.name,"page_count" to uris.size));val e=exam;if(e!=null&&uris.isNotEmpty()){saving=true;when(pending){ScanMode.Single->vm.addSingle(examId,uris,{saving=false;Diagnostics.log(DiagnosticCategory.ML_KIT,"scan_saved",mapOf("mode" to "single","page_count" to uris.size))},{saving=false;error=it.message});ScanMode.Bulk->vm.addBulk(examId,e.pagesPerPaper,uris,{saving=false;bulkActive=false;Diagnostics.log(DiagnosticCategory.ML_KIT,"bulk_capture_finished",mapOf("page_count" to uris.size))},{saving=false;bulkActive=false;error=it.message});null->{saving=false}}}}else{if(pending==ScanMode.Bulk)bulkActive=false;Diagnostics.log(DiagnosticCategory.ML_KIT,"scanner_cancelled",mapOf("result_code" to r.resultCode,"bulk_session_ended" to (pending==ScanMode.Bulk)))}}
    fun scan(mode:ScanMode){val e=exam?:return;if(mode==ScanMode.Bulk)bulkActive=true;pending=mode;val limit=if(mode==ScanMode.Single)e.pagesPerPaper else 200;Diagnostics.log(DiagnosticCategory.ML_KIT,"scanner_requested",mapOf("mode" to mode.name,"page_limit" to limit));val options=GmsDocumentScannerOptions.Builder().setGalleryImportAllowed(true).setPageLimit(limit).setResultFormats(GmsDocumentScannerOptions.RESULT_FORMAT_JPEG).setScannerMode(GmsDocumentScannerOptions.SCANNER_MODE_FULL).build();GmsDocumentScanning.getClient(options).getStartScanIntent(activity).addOnSuccessListener{Diagnostics.log(DiagnosticCategory.ML_KIT,"scanner_ready");launcher.launch(IntentSenderRequest.Builder(it).build())}.addOnFailureListener{bulkActive=false;Diagnostics.log(DiagnosticCategory.ML_KIT,"scanner_launch_failed",error=it);error=it.message?:"Scanner unavailable"}}
    val canExport=ExamUiRules.canExport(papers.map{it.pages.size})
    fun export(type:String){exporting=true;scope.launch{try{val uri=if(type=="pdf")vm.exportPdf(examId) else vm.exportZip(examId);readyExport=ReadyExport(uri,type,if(type=="pdf")"application/pdf" else "application/zip",if(type=="pdf")"ExamScan.pdf" else "ExamScan-images.zip");Diagnostics.log(DiagnosticCategory.SHARING,"export_ready",mapOf("type" to type))}catch(t:Throwable){Diagnostics.log(DiagnosticCategory.STORAGE,"export_failed",mapOf("type" to type),t);error=t.message?:"Export failed"}finally{exporting=false}};menu=false}
    Scaffold(topBar={TopAppBar(title={Column{Text(exam?.name?:"Exam",fontWeight=FontWeight.Bold);exam?.let{Text("${it.examDate} • ${papers.size} papers",style=MaterialTheme.typography.labelSmall)}}},navigationIcon={IconButton({nav.popBackStack()}){Icon(Icons.Default.ArrowBack,null)}},actions={IconButton({menu=true},modifier=Modifier.testTag("export_menu_button")){Icon(Icons.Default.MoreVert,null)};DropdownMenu(menu,{menu=false}){DropdownMenuItem({Text("Export PDF")},{export("pdf")},leadingIcon={Icon(Icons.Default.PictureAsPdf,null)},enabled=canExport,modifier=Modifier.testTag("export_pdf_button"));DropdownMenuItem({Text("Export images ZIP")},{export("zip")},leadingIcon={Icon(Icons.Default.FolderZip,null)},enabled=canExport,modifier=Modifier.testTag("export_zip_button"))}})},floatingActionButton={Column(horizontalAlignment=Alignment.End,verticalArrangement=Arrangement.spacedBy(10.dp)){SmallFloatingActionButton({scan(ScanMode.Bulk)},modifier=Modifier.testTag("bulk_scan_button")){Icon(Icons.Default.Collections,null)};ExtendedFloatingActionButton(text={Text("Scan paper")},icon={Icon(Icons.Default.DocumentScanner,null)},onClick={scan(ScanMode.Single)},modifier=Modifier.testTag("single_scan_button"))}}){pad->
        if(papers.isEmpty()) EmptyState("Ready to scan","Use Scan paper for one submission or the upper button for a continuous bulk session.",Modifier.padding(pad)) else LazyColumn(Modifier.padding(pad).fillMaxSize(),contentPadding=PaddingValues(16.dp,16.dp,16.dp,120.dp),verticalArrangement=Arrangement.spacedBy(12.dp)){items(papers,key={it.paper.id}){pw->if(exam?.pagesPerPaper==1)SinglePageCard(pw,{nav.navigate("paper/${pw.paper.id}/1")},{vm.deletePaper(pw.paper)})else PaperCard(pw,exam?.pagesPerPaper?:0,{nav.navigate("paper/${pw.paper.id}/${exam?.pagesPerPaper?:1}")},{vm.deletePaper(pw.paper)})}}
    }
    ...
    if(saving)Box(Modifier.fillMaxSize().background(androidx.compose.ui.graphics.Color.Black.copy(alpha=.35f)),contentAlignment=Alignment.Center){Card{Column(Modifier.padding(24.dp),horizontalAlignment=Alignment.CenterHorizontally){CircularProgressIndicator();Spacer(Modifier.height(12.dp));Text(if(bulkActive)"Saving and grouping bulk pages…" else "Saving scan…")}}}
    if(exporting)AlertDialog(onDismissRequest={},confirmButton={},title={Text("Preparing export")},text={Column(horizontalAlignment=Alignment.CenterHorizontally){CircularProgressIndicator();Spacer(Modifier.height(12.dp));Text("Creating and checking the file…")}})
    readyExport?.let{item->AlertDialog(onDismissRequest={readyExport=null},title={Text("Export ready")},text={Text("Save the file to your device or share it with another app.")},confirmButton={Button({share(activity,item.uri,item.mime);readyExport=null}){Text("Share")}},dismissButton={Row{TextButton({val intent=Intent(Intent.ACTION_CREATE_DOCUMENT).apply{addCategory(Intent.CATEGORY_OPENABLE);type=item.mime;putExtra(Intent.EXTRA_TITLE,item.fileName)};saveLauncher.launch(intent)}){Text("Save")};TextButton({readyExport=null}){Text("Close")}}})}
}

enum class ScanMode{Single,Bulk}
data class ReadyExport(val uri:Uri,val type:String,val mime:String,val fileName:String)

@Composable fun SinglePageCard(pw:PaperWithPages,open:()->Unit,delete:()->Unit){Card(onClick=open,modifier=Modifier.fillMaxWidth()){Row(Modifier.padding(14.dp),verticalAlignment=Alignment.CenterVertically){pw.pages.firstOrNull()?.let{AsyncImage(File(it.labeledPath),"Scan ${pw.paper.paperNumber}",Modifier.size(76.dp,100.dp).clip(RoundedCornerShape(8.dp)),contentScale=ContentScale.Crop)};Spacer(Modifier.width(14.dp));Column(Modifier.weight(1f)){Text("Scan ${pw.paper.paperNumber}",fontWeight=FontWeight.Bold);Text("1 page",style=MaterialTheme.typography.bodySmall)};ConfirmDeleteButton("Delete scan",delete)}}}
...
@Composable fun PageEditorCard(p:PageEntity,retake:()->Unit,insertAfter:()->Unit,delete:()->Unit,edit:()->Unit={},canInsert:Boolean=true){Card{Column{AsyncImage(File(p.labeledPath),"Page ${p.pageNumber}",Modifier.fillMaxWidth().aspectRatio(0.707f).clickable(onClick=edit),contentScale=ContentScale.Fit);Row(Modifier.fillMaxWidth().padding(8.dp),verticalAlignment=Alignment.CenterVertically){Text("Page ${p.pageNumber}",fontWeight=FontWeight.Bold,modifier=Modifier.weight(1f));IconButton(edit){Icon(Icons.Default.Edit,"Edit image")};TextButton(retake){Icon(Icons.Default.Refresh,null);Spacer(Modifier.width(4.dp));Text("Retake")};IconButton(insertAfter,enabled=canInsert){Icon(Icons.Default.PlaylistAdd,"Insert after")};IconButton(delete){Icon(Icons.Default.DeleteOutline,"Delete")}}}}}

@OptIn(ExperimentalMaterial3Api::class)
@Composable fun PageImageEditor(vm:AppViewModel,nav:NavHostController,pageId:Long){val page by vm.pages(0).collectAsState(initial=emptyList());var rotation by remember{mutableIntStateOf(0)};var filter by remember{mutableStateOf("ORIGINAL")};var crop by remember{mutableFloatStateOf(0f)};var saving by remember{mutableStateOf(false)};var error by remember{mutableStateOf<String?>(null)};val context=androidx.compose.ui.platform.LocalContext.current;var file by remember{mutableStateOf<File?>(null)};LaunchedEffect(pageId){val app=context.applicationContext as ExamScanApplication;file=app.database.pageDao().get(pageId)?.let{File(it.originalPath)}};Scaffold(topBar={TopAppBar(title={Text("Edit captured page")},navigationIcon={IconButton({nav.popBackStack()}){Icon(Icons.Default.ArrowBack,null)}})},bottomBar={BottomAppBar{Button({saving=true;vm.applyEdits(pageId,rotation,filter,crop.toInt(),{saving=false;nav.popBackStack()},{saving=false;error=it.message})},Modifier.fillMaxWidth().padding(horizontal=16.dp),enabled=!saving){Text("Apply edits")}}}){pad->Column(Modifier.padding(pad).fillMaxSize().padding(16.dp),verticalArrangement=Arrangement.spacedBy(12.dp)){AsyncImage(file,null,Modifier.weight(1f).fillMaxWidth().graphicsLayer(rotationZ=rotation.toFloat()),contentScale=ContentScale.Fit,colorFilter=when(filter){"GRAYSCALE"->ColorFilter.colorMatrix(ColorMatrix().apply{setToSaturation(0f)});"HIGH_CONTRAST"->ColorFilter.colorMatrix(ColorMatrix(floatArrayOf(1.5f,0f,0f,0f,-.25f,0f,1.5f,0f,0f,-.25f,0f,0f,1.5f,0f,-.25f,0f,0f,0f,1f,0f)));else->null});Row(horizontalArrangement=Arrangement.spacedBy(8.dp)){FilterChip(filter=="ORIGINAL",{filter="ORIGINAL"},{Text("Original")});FilterChip(filter=="GRAYSCALE",{filter="GRAYSCALE"},{Text("Grayscale")});FilterChip(filter=="HIGH_CONTRAST",{filter="HIGH_CONTRAST"},{Text("Contrast")})};Row(verticalAlignment=Alignment.CenterVertically){OutlinedButton({rotation=(rotation+270)%360}){Text("↶ Rotate")};Spacer(Modifier.width(8.dp));OutlinedButton({rotation=(rotation+90)%360}){Text("Rotate ↷")}};Text("Crop edges: ${crop.toInt()}%");Slider(crop,{crop=it},valueRange=0f..25f,steps=4)}};if(saving)AlertDialog(onDismissRequest={},confirmButton={},title={Text("Applying edits")},text={CircularProgressIndicator()});error?.let{AlertDialog({error=null},confirmButton={TextButton({error=null}){Text("OK")}},title={Text("Edit failed")},text={Text(it)})}}
