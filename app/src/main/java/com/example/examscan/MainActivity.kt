...
@Composable fun HomeScreen(vm:AppViewModel,nav:NavHostController){
    ...
    Scaffold(topBar={TopAppBar(title={Column{Text("ExamScan",fontWeight=FontWeight.Bold);Text("Fast exam paper capture",style=MaterialTheme.typography.labelMedium)}})},floatingActionButton={ExtendedFloatingActionButton(onClick={show=true},icon={Icon(Icons.Default.Add,null)},text={Text("New exam")})}){pad->
...
@Composable fun PaperScreen(vm:AppViewModel,nav:NavHostController,paperId:Long,expected:Int){
    ...
    Scaffold(topBar={TopAppBar(title={Column{Text("Paper editor",fontWeight=FontWeight.Bold);Text("${pages.size}/$expected pages",style=MaterialTheme.typography.labelSmall)}},navigationIcon={IconButton({nav.popBackStack()}){Icon(Icons.Default.ArrowBack,null)}})},floatingActionButton={ExtendedFloatingActionButton({scan(PageAction.Insert(pages.size))},icon={Icon(Icons.Default.Add,null)},text={Text("Add page")})}){pad->
