...
@Composable fun HomeScreen(vm:AppViewModel,nav:NavHostController){
    ...
    Scaffold(topBar={TopAppBar(title={Column{Text("ExamScan",fontWeight=FontWeight.Bold);Text("Fast exam paper capture",style=MaterialTheme.typography.labelMedium)}})},floatingActionButton={ExtendedFloatingActionButton(text={Text("New exam")},icon={Icon(Icons.Default.Add,null)},onClick={show=true},modifier=Modifier.testTag("create_exam_button"))}){pad->
...
@Composable fun ExamScreen(vm:AppViewModel,nav:NavHostController,examId:Long){
    ...
    Scaffold(topBar={TopAppBar(title={Column{Text(exam?.name?:"Exam",fontWeight=FontWeight.Bold);exam?.let{Text("${it.examDate} • ${papers.size} papers",style=MaterialTheme.typography.labelSmall)}}},navigationIcon={IconButton({nav.popBackStack()}){Icon(Icons.Default.ArrowBack,null)}},actions={IconButton({menu=true},modifier=Modifier.testTag("export_menu_button")){Icon(Icons.Default.MoreVert,null)};DropdownMenu(menu,{menu=false}){DropdownMenuItem({Text("Export PDF")},{scope.launch{share(activity,vm.exportPdf(examId),"application/pdf")};menu=false},leadingIcon={Icon(Icons.Default.PictureAsPdf,null)},enabled=canExport,modifier=Modifier.testTag("export_pdf_button"));DropdownMenuItem({Text("Export images ZIP")},{scope.launch{share(activity,vm.exportZip(examId),"application/zip")};menu=false},leadingIcon={Icon(Icons.Default.FolderZip,null)},enabled=canExport,modifier=Modifier.testTag("export_zip_button"))}})},floatingActionButton={Column(horizontalAlignment=Alignment.End,verticalArrangement=Arrangement.spacedBy(10.dp)){SmallFloatingActionButton({scan(ScanMode.Bulk)},modifier=Modifier.testTag("bulk_scan_button")){Icon(Icons.Default.Collections,null)};ExtendedFloatingActionButton(text={Text("Scan paper")},icon={Icon(Icons.Default.DocumentScanner,null)},onClick={scan(ScanMode.Single)},modifier=Modifier.testTag("single_scan_button"))}}){pad->
